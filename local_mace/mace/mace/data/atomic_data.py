...
class AtomicData(torch_geometric.data.Data):
    ...
    def from_config(
        ...
        if not config.interlayer_ml:
            ...
            edge_index, shifts, unit_shifts = get_neighborhood_layered(
                positions=config.positions,
                cutoff=cutoff,
                pbc=config.pbc,
                cell=config.cell,
                atomic_numbers=config.atomic_numbers,  # Requires atomic nums for bond length defaults
            )
