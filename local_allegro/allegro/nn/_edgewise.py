...
class EdgewiseEnergySum(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        if normalize_edge_energy_sum and avg_num_neighbors is not None:
            self._factor = 1.0 / math.sqrt(avg_num_neighbors)
        # AR quick fix to implement weights
        # The yaml file now takes per_edge_species to be a list of lists
        # This is of size ntypes x ntypes
        if per_edge_species_scale:
            self.per_edge_species_scale = True
            self.register_buffer(
                "per_edge_scales", torch.Tensor(per_edge_species_scale)
            )
        else:
            self.per_edge_species_scale = False
