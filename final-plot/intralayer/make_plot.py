...
def make_plot():
    ...
    atoms_mace = read('mace-predicted.xyz', index=":", format="extxyz")

    atoms = read('actual-data.xyz', index=":", format="extxyz")
    ...
    with open('uf23-predicted.json', 'r') as file:
    ...
    with open('uf23-actual.json', 'r') as file:
        actual_uf3 = np.array(json.load(file))

    print(np.abs(predicted_allegro - actual_values))
    # Calculate residuals
    residuals_allegro = np.log(np.abs(predicted_allegro - actual_values))
    residuals_mace = np.log(np.abs(predicted_mace - actual_values))
    ...
    plt.title('Box Plot of Residuals for Multiple Models on Intralayer Dataset', pad=20)
    plt.grid(True)
